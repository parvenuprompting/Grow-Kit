...
struct ContentView: View {
    ...
    enum Modi: Int, CaseIterable, Identifiable {
        ...
        var naam: String {
            ...
            case .kloon: return "Digitale Kloon"
        ...
        var beschrijving: String {
            ...
            case .kloon: return "Je persoonlijke kluis — wachtwoorden, keys, bank en crypto"
    ...
    var body: some View {
        ...
        .onAppear { Thema.registreerFonts(); laadSaldo(); startSaldoTimer(); statusbalkStore.start()
            ...
            AgentVoorlader.gedeeld.start(repoPad: repoPad, interpreter: interpreter, runner: runner) }
        .sheet(isPresented: $toonInstellingen) { InstellingenView() }
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    ForEach([Modi.securevault, .amnesia, .kloon, .hervatten, .taak]) { modus in
                        modusRij(modus)
                    }
    ...
    private var schermVoet: some View {
        HStack {
            HStack(spacing: 6) {
                Text("© Parvenu GrowKit 1.5.0 · Zero-Trust Harnas — ontwikkeld door Tiëndo Welles")
            }
            Spacer()
        }
