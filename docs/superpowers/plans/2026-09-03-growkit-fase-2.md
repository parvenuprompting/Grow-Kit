...
4. **Bekende valkuilen:** (a) clone-test mag niet per ongeluk de werkkopie gebruiken — clone-pad geforceerd tmp; (b) poort mag nooit tekst gaan interpreteren — alleen veldaanwezigheid; (c) herstart-test (1.6) kan breken op de `idempotent: false`-stap-005 — verwacht gedrag vastleggen vóór de run; (d) weigeringsteksten zijn vaste constanten, geen gegenereerde zinnen — anders is de tekst-vergelijkingstest nep.
